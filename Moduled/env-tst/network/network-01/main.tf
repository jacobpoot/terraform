...
resource "azurerm_subnet" "gateway-subnet" {
  ...
  address_prefix       = "10.10.240.0/24"
}

resource "azurerm_subnet" "sql-subnet" {
  name                 = "10.10.1.0-28-SQL-SUBNET"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
...
resource "azurerm_subnet" "monitoring-subnet" {
  name                 = "10.10.2.0-28-MONITORING-SUBNET"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.10.2.0/28"
}

resource "azurerm_subnet" "bi-subnet" {
  name                 = "10.10.3.0-28-BI-SUBNET"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.10.3.0/28"
}

resource "azurerm_subnet" "hrm-subnet" {
  name                 = "10.10.4.0-28-HRM-SUBNET"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.10.4.0/28"
}
