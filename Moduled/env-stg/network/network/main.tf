resource "azurerm_resource_group" "network" {
  name     = "TAS-EUW-RGR-TST-TAS-NETWORK-01"
  location = "${var.location}"
}

resource "azurerm_virtual_network" "network" {
  name                = "TAS-EUW-VNT-TAS-NETWORK-01"
...
resource "azurerm_subnet" "gateway-subnet" {
  ...
  address_prefix       = "10.151.255.192/26"
}
...
resource "azurerm_subnet" "application-gateway-subnet" {
  ...
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.255.128/26"
}

resource "azurerm_subnet" "prd-ogd" {
  name                 = "10.151.1.16-28-PRD-OGD"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.1.0/28"
}

resource "azurerm_subnet" "prd-rcz" {
  name                 = "10.151.2.16-28-PRD-RCZ"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.2.0/28"
}

resource "azurerm_subnet" "prd-srl" {
  name                 = "10.151.3.16-28-PRD-SRL"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.3.0/28"
}

resource "azurerm_subnet" "prd-kvl" {
  name                 = "10.151.4.16-28-PRD-KVL"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.4.0/28"
}

resource "azurerm_subnet" "stg-ogd" {
  name                 = "10.151.1.0-28-STG-OGD"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.1.0/28"
}

resource "azurerm_subnet" "stg-rcz" {
  name                 = "10.151.2.0-28-STG-RCZ"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.2.0/28"
}

resource "azurerm_subnet" "stg-srl" {
  name                 = "10.151.3.0-28-STG-SRL"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.3.0/28"
}

resource "azurerm_subnet" "stg-kvl" {
  name                 = "10.151.4.0-28-STG-KVL"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.4.0/28"
}

resource "azurerm_subnet" "prd-stg" {
  name                 = "10.151.5.16-28-STG-STG"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.151.4.0/28"
}
...
resource "azurerm_local_network_gateway" "local-network-gateway-01" {
  name                = "TAS-EUW-LNG-TAS-KLANTENPLEIN-01"
  resource_group_name = "${azurerm_resource_group.network.name}"
