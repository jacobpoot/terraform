...
resource "azurerm_subnet" "gateway-subnet" {
  ...
  address_prefix       = "10.66.1.240/28"
}

resource "azurerm_subnet" "prd-etl" {
  name                 = "10.66.1.16-28-PRD-ETL"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.66.1.16/28"
}

resource "azurerm_subnet" "prd-dtm" {
  name                 = "10.66.1.32-28-PRD-DTM"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.66.1.32/28"
}

resource "azurerm_subnet" "prd-rpl" {
  name                 = "10.66.1.48-28-PRD-RPL"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.66.1.48/28"
}

resource "azurerm_subnet" "prd-ptg" {
  name                 = "10.66.1.64-28-PRD-PTG"
  resource_group_name  = "${azurerm_resource_group.network.name}"
  virtual_network_name = "${azurerm_virtual_network.network.name}"
  address_prefix       = "10.66.1.64/28"
}
